...
class admin_ufield extends core_control {

    public $_tpl = '';

    public function init() {
        parent::init();
        $this->_table = table_ufields::getInstance();
        
        $this->umodel = table_umodel::getInstance();
        $row = $this->umodel->getrow(array('modelid'=>core_app::$get['modelid']));
        $this->tpl->umodel = $row;
        
        $this->umodeltable = new table_ufields();
        $this->umodeltable->name = $this->umodeltable->prefix.$row['tbname'];
    ...
    public function add_act() {
        if (core_app::$post['sub']) {
            ...
            if($this->_table->rec_insert(core_app::$post)){
                ...
                $sql = "ALTER TABLE `".$this->umodeltable->name."` ADD `".core_app::$post['name']."` ".$type.";";
                $this->_table->query($sql);
                if(core_app::$post['isunique']){
                    $sql = "ALTER TABLE `".$this->umodeltable->name."` ADD UNIQUE(`".core_app::$post['name']."`);";
                    $this->_table->query($sql);
                }
                core_url::closeiframe(true);
            }else{
    ...
    public function del_act() {
        if (core_app::$get['fieldid']) {
            ...
            $sql = "ALTER TABLE `".$this->umodeltable->name."` DROP `".core_app::$get['fieldname']."`";
            $this->_table->query($sql);
            core_url::alertinfo('删除字段成功', url('ufield','list',1,array('modelid'=>core_app::$get['modelid'])));
        }
